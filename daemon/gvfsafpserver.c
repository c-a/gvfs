                        ...
                        GError         **error)
{
  ...
  while (TRUE)
  {
    ...
    if (initial_user)
      ...
      g_string_append_printf (str, _("Enter password for afp as %s on %s"), initial_user, priv->info.server_name);
    else
      /* translators: %s here is the hostname */
      g_string_append_printf (str, _("Enter password for afp on %s"), priv->info.server_name);
